...
def sor_approx_conditional_probability_iteration_exp(G, beta, gamma, initial_infected_nodes, tol=0.001, omega=1.3):
    N = G.number_of_nodes()
    status = np.zeros((N, 3))
    ...
    while max(i) > tol:
        ...
        for node in G.nodes():
            ...
            s[node] = s_new
            i[node] = omega*i_new + (1-omega)*i[node]
            r[node] = r_new
...
def test_conditional_probability_iteration_estimation():
    ...
    results = sor_approx_conditional_probability_iteration_exp(G, beta, gamma, initial_infected_nodes, tol, 1.3)
